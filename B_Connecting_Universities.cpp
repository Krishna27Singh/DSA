...
void dfs2 (ll v, ll par = -1, ll dec = 0){
  ll idx = -1;
  ll cc = cnt[v] - dec;
 
  for (auto u : adj[v]){
    if (u != par && (idx == -1 || cnt[u] > cnt[idx])) idx = u;
  }
  ...
  if (sz * 2 <= cc){
    ...
    //  cout << v + 1 << ' ' << h[v] << ' ' << cc << endl;
    return ;
  }
 
  // cout << v + 1 << ' ' << h[v] << ' ' << 2 * (cc - cnt[u]) << endl;
...
void solve(){
  ...
  //  for (int i = 0; i < n; i ++) cout << i + 1 << ' ' << cnt[i] << endl;
