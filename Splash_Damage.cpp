...
void solve(){
    ...
    rep(i, 1, n){
        vi ndp(3, -INF);
        int p = a[i-1];
        if(dp[0] >= 0) ndp[0] = max(ndp[0], dp[0]);
        if(dp[1] >= 0) ndp[0] = max(ndp[0], dp[1]);
        if(p>=2 && dp[2] >= 0) ndp[0] = max(ndp[0], dp[2]);
        if(dp[0] >= 0) ndp[1] = max(ndp[1], dp[0]+1);
        if(p>=2 && dp[1] >= 0) ndp[1] = max(ndp[1], dp[1]+1);
        if(p == 3 && dp[2] >= 0) ndp[1] = max(ndp[1], dp[2]+1);
        if(dp[1]>=0) ndp[2] = max(ndp[2], dp[1]+1);
        if(p>=2 && dp[2]>=0) ndp[2] = max(ndp[2], dp[2]+1);
        dp = ndp;
    }

    int ans = max(dp[0], dp[1]);
    if(a[n-1] >= 2) ans = max(ans, dp[2]);
    cout<<ans<<endl;
