...
void dfs(ll node, vector<vector<ll>> &adj, vector<ll> &ss, vector<int> &vis){
    vis[node] = 1;
    ll subSize = 1;
    for(auto child : adj[node]){
        if(!vis[child]){
            dfs(child, adj, ss, vis);
            subSize += ss[child];
        }
    }
    ss[node] = subSize;
...
void solve(){
    ...
    for(ll node = 1; node <n; node++){
        auto si = ss[node];
        num = (num + (nCr(si, k/2, fact, invFact)%MOD) * (nCr(n-si, k/2, fact, invFact)%MOD))%MOD;
    }
    ll den = nCr(n, k, fact, invFact)%MOD;
    ll ans = (1+num * modInverse(den))%MOD;
    cout<<ans<<endl;
