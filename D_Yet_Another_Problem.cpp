...
void solve(){
    int n, q; cin >> n >> q;
	vector<int> a(n + 1), px(n + 1, 0), pz(n + 1, 0); 
	for (int i = 1; i <= n; i++) {
		cin >> a[i];
		px[i] = px[i - 1] ^ a[i]; 
		pz[i] = pz[i - 1] + (a[i] != 0); 
	}
    unordered_map<int, vector<int>> e, o; 

	for (int i = 0; i <= n; i++) {
		if (i%2 == 0) e[px[i]].push_back(i); 
		else o[px[i]].push_back(i); 
	}
    while (q--) {
        int l, r; cin >> l >> r;
        if (pz[r] - pz[l - 1] == 0) {
        ...
        vector<int>* vec;
        if(l%2 == 0) vec = &e[px[l-1]];
        else vec = &o[px[l-1]]; 
		auto it = lower_bound(vec->begin(), vec->end(), l);
        if (it != vec->end() && *it <= r - 1) cout << 2 << endl;
		else cout << -1 << endl;
	}
