...
void solve(){
    ...
    vb checkc1(k + 1);
    while(q--) {
        int idx, pos; cin>>idx>>pos;
        add(S[idx], -1);
        S[idx][pos - 1] = (S[idx][pos - 1] == 'o' ? 'x' : 'o');
        add(S[idx], 1);
        ...
        rep(step, 1, k + 1) {
            int posb = (T[step - 1] == 'x' ? 1 : 0);
            int fnode = (u ? (posb == 0 ? ch0[u] : ch1[u]) : 0);
            int kC = (fnode ? cntn[fnode] : 0);

            if(cntq + kC <= m) {
                checkc1[step] = true;
                cntq += kC;
                P += (posb == 0 ? 'x' : 'o');
                u = (u ? (posb == 0 ? ch1[u] : ch0[u]) : 0);
            } 
            else{
                checkc1[step] = false;
                P += (posb == 0 ? 'o' : 'x');
        ...
        int mm = -1;
        rep(step, 1, k + 1) {
            if (S[idx][step - 1] != P[step - 1]) {
                mm = step;
                break;
            }
        }

        if(mm != -1 && checkc1[mm]) cout << "Yes" << endl;
        else cout << "No" << endl;
    }
}
