...
void solve(){
    ...
    for (int i = 0; i < n; ++i) {
        vector<vector<ll>> dp1(k + 1, vector<ll>(k + 1, -INF));
        for (int j = 0; j <= k; ++j) {
            for (int l = j; l <= k; ++l) {
                if (dp[j][l] == -INF) continue;
                for (int s = 0; l + s <= k; ++s) {
                    dp1[j + s][l + s] = max(dp1[j + s][l + s], dp[j][l]);
                }
            }
        }
        vector<vector<ll>> dp2(k + 1, vector<ll>(k + 1, -INF));
        for (int c = 0; c <= k; ++c) {
            ll val = (c % 2 == 1) ? b[i] : a[i];
            for (int l = c; l <= k; ++l) {
                if (dp1[c][l] != -INF) {
                    dp2[c][l] = dp1[c][l] + val;
                }
        ...
        vector<vector<ll>> dp3(k + 1, vector<ll>(k + 1, -INF));
        for (int c = 0; c <= k; ++c) {
            for (int l = c; l <= k; ++l) {
                if (dp2[c][l] == -INF) continue;
                for (int e = 0; e <= c; ++e) {
                    dp3[c - e][l] = max(dp3[c - e][l], dp2[c][l]);
                }
            }
        }
        dp = dp3;
