...
void solve(){
    ...
    int l = div.size();
    vector<ll> h(l);
    for(int i = 0; i<l; i++) h[i] = div[l-1-i];
    vector<ll> pr(l+1, 0);
    for(int i = 0; i<l; i++){
        if(i>0) pr[i] = pr[i-1] + (div[i]-div[i-1])*h[i];
        else pr[i] = (div[i])*h[i];
    }
    ...
    while(q--){
        ...
        int x1 = lower_bound(div.begin(), div.end(), x) - div.begin();

        ll ans = 0;
        if(min(y1, x1-1) >= 0) ans += div[min(x1-1, y1)]*y;

        if(x1-1 > min(y1, x1-1)){
            if(x1-1 >= 0) ans += pr[x1-1];
            if(min(y1, x1-1)>=0) ans -= pr[min(y1, x1-1)];
        }
        if(x1 > 0) ans += (x-div[x1-1])*min(h[x1], y);
        else ans += x*min(h[x1], y);
        cout<<ans<<endl;
    }
